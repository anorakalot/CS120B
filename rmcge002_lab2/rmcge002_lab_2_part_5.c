...
int main(void)
{
	...
	while(1){
		...
		// If the weight is above 5 but below 70, the airbag should be disabled and an "Airbag disabled" icon should light by setting PB2 to 1. /*Checking if it's above 5*/  ((   (tmpD>=0x02)&&(GetBit(tmpB_input,0))    /*|| (tmpD>0x05)) && Checking if below 70/ ((tmpD<0x23) && ((tmpD< 0x22) ||GetBit(tmpB_input,0) )) ))*/   
		else if( (( tmpB_input == 0x01  && tmpD > 0x02) || (tmpB_input == 0x00 && tmpD >= 0x03))) {
